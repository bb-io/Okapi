...
{
    [Action("Import TMX to TM", Description = "Import TMX file to create or update a translation memory.")]
    public async Task<CreateTmResponse> CreateTm([ActionParameter] CreateTmRequest request)
    {
        ...
        FileParameter tmxFileParam = FileParameter.Create(
            "inputFile",
            () => fileManagementClient.DownloadAsync(request.Tmx).GetAwaiter().GetResult(),
            request.Tmx.Name,
    ...
    [Action("Download TM", Description = "Export a translation memory content as archived Pensieve TM (could be locally exported to a bilingual TMX file using OKAPI Rainbow).")]
    public Task<ExportTmxResponse> DownloadTm([ActionParameter] ExportTmxRequest request)
    {
        ...
        // Pensieve TM can't be convered using OKAPI Longhorn.
    ...
    [Action("Update TM", Description = "Update translation memory from a bilingual file content.")]
    public async Task<UpdateTmResponse> UpdateTm([ActionParameter] UpdateTmRequest request)
    {
        ...
        var importConfigOverwrite = BatchConfig.OverrideTmImportConfig(request.TMPath, request.OverwriteSameSource == true);
        var projectId = await CreateProjectWithBatchConfig("import_to_tm", importConfigOverwrite);
        ...
        await UploadFile(projectId, FileParameter.Create("inputFile", xliffBytes, "update.xliff", "application/xliff+xml"));
